...
| 44 | L | 60+ historical `.bin` files (incl. ESP8266) committed in `firmware/`; `esphome.html` is an ESP Web Tools snippet, not ESPHome. README says `compress_html.py` but the script is `pre_build.py`. | Docs corrected; historical binaries, the ESP8266 tree, the v2/experimental manifests and `esphome.html` removed from the tree (they remain in git history and on GitHub Releases); `firmware/` now holds only the current v3 image, its OTA file and one manifest. |
